...
- 使用“白色内衬 + 四角短线”的相框形式，增强边界但不引入连续外框、阴影、圆角或厚重拟物感。
...
- 相框内部保留 6px 白色内衬，四角使用 `#cfcfcf` 的 1px L 形短线形成清晰但轻量的边界。
...
进入 story 后，页面像一张纯白背景上的摄影画册：没有任何站点级导航，标题区紧凑，正文与相册同宽，相框边界清晰但不厚重，底部只保留一个易识别的返回箭头作为退出动作。打开照片后，序号回到左上角，左右切换足够轻、不遮挡照片，右上角不出现关闭按钮，同时仍可通过背景、Esc 或垂直拖动退出。
