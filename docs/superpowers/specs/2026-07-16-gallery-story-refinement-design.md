...
- 使用暖象牙白展墙、深石墨金属细框、博物馆白卡纸和轻微悬浮阴影，形成克制的当代摄影展墙。
...
- 标题、metadata、简介和支持的文本容器都与相册使用同一个媒体宽度变量，字号为 15px，行高为 1.65。
- 标题、metadata 和简介左对齐，与相册左边界对齐；不使用额外卡片或装饰线。

### 相框网格

- 相框根据照片原始宽高比保留不同画幅节奏，图片使用 `object-fit: cover`，不做全出血。
- 相框使用深石墨色 `1px` 金属细框，内部使用白色卡纸留白，不使用木框、圆角或厚重装饰。
- 阴影保持在很低的对比度，仅用于让相框从墙面轻微浮起；图片本身不增加 hover 缩放。
- 网格保持整齐的横向和纵向间距，作品描述直接位于相框下方，仅显示小号灰黑文字和一条细横线，不显示序号、卡片底板或额外阴影。
...
- 计数器恢复 PhotoSwipe 默认的左上角位置，使用低调小字，不与箭头形成视觉重心。
...
- dev server 检查 story 页面：无顶部导航、正文宽度与相册一致、四角短线相框边界清晰、返回图标可识别、lightbox 序号位于左上角、箭头不遮挡图片且无右上角关闭按钮。

## 验收标准

进入 story 后，页面像暖象牙白展墙上的当代摄影展：没有任何站点级导航，标题区紧凑，正文与相册同宽，不同原始画幅形成有秩序的节奏，深石墨细框和白卡纸清晰但不厚重，展签只保留文字与细横线，底部只保留一个易识别的返回箭头作为退出动作。打开照片后，序号回到左上角，左右切换足够轻、不遮挡照片，右上角不出现关闭按钮，同时仍可通过背景、Esc 或垂直拖动退出。
