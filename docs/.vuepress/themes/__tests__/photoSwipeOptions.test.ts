...
describe('galleryPhotoSwipeOptions', () => {
  ...
  it('scopes album-only PhotoSwipe controls to the StoryAlbum main class', () => {
    ...
    const storyCounterRule = styles.match(/\.pswp\.story-album-lightbox \.pswp__counter\s*\{([^}]*)\}/)?.[1] ?? ''
    expect(storyCounterRule).not.toMatch(/(?:^|;)\s*(?:position|top|right|bottom|left|text-align)\s*:/)
